resource "signalfx_list_chart" "unknown_sync_list" {
  name = "Apps with UNKNOWN Sync Status"
  description = "List of apps with UNKNOWN Sync Status"
  program_text = <<-EOF
B = data('argocd_app_info', filter=filter('sync_status', 'Unknown'), rollup='average').publish(label='B')
EOF

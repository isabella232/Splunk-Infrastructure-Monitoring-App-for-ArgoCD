resource "signalfx_list_chart" "unknown_health_list" {
  name = "Apps with UNKNOWN Health Status"
  description = "List of apps with UNKNOWN Health Status"
  program_text = <<-EOF
B = data('argocd_app_info', filter=filter('health_status', 'Unknown'), rollup='average').publish(label='B')
EOF

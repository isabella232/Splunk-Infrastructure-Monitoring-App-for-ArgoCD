resource "signalfx_list_chart" "missing_health_list" {
  name = "Missing Health"
  description = "List of apps with MISSING Health Status"
  program_text = <<-EOF
B = data('argocd_app_info', filter=filter('health_status', 'Missing'), rollup='average').publish(label='B')
EOF
